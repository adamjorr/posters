...
\begin{center}
	\begin{tikzpicture}[align=center,cnode/.style={rectangle,draw=black!60,fill=black!5,very thick}, node distance = -.5cm and 4 cm]
		\node[cnode] (reads){Reads};
		\node[cnode] (em1)[above right = of reads]{\textit{E. melliodora} 1 (BWA)};
		\node[cnode] (em2)[below right = of reads]{\textit{E. melliodora} 1 (NGM)};
		\node[cnode] (bwaq)[right = 2cm of em1]{Alignment Quality Scores};
		\node[cnode] (ngmq)[right = 2cm of em2]{Alignment Quality Scores};
		\draw[line width=3mm,->] (reads.north) |- (em1.west) node[pos=.7,above]{Bowtie};
		\draw[line width=3mm,->] (reads.south) |- (em2.west) node[pos=.7,below]{Bowtie};
